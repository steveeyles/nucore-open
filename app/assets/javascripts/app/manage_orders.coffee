...
class OrderDetailManagement
  ...
  initPriceUpdating: ->
    self = this
    @$element.find('[name^="order_detail[reservation]"]:not([name$=_display]),[name="order_detail[quantity]"],[name="order_detail[account_id]"],[name="order_detail[fulfilled_at]"]').bind "change keyup", (evt) ->
